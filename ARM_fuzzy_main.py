...
"""
This is a MODIFIED VERSION of the orignal Fuzzy-logic code from Elizabeth Smith, 
which has been modified to use ARM data rather than data from CLAMPS.
Her original code can be found at: https://github.com/eeeeelizzzzz/bliss-fl
"""
...
def HF_varcalc(field, time, height):
  ...
  HFsigw, HFsigw_t = calcSigma(time, height, filt_field, window=.166) #computing the 
...
def Pot_Temp(temperature,pressure):
    
    """
    This function solves for potential temperature, since metpy wants units
    and the AREI data from ARM doesn't have it
    
    Args:
        temperature (array): 2-D array of temps [kelvin]
        pressure (array): 2-D array of pressures [kPa]
    Returns:
        potential_temperature (array): 2-D array of potential temperature
    """
    
    potential_temperature = np.full((len(temperature[:,0]),len(temperature[0,:])),np.nan, np.double)
    p0 = 100
    for t in range (0,len(temperature[:,0])):
        for z in range (0,len(temperature[0,:])):
            T = temperature[t,z]
            P = pressure[t,z]
            theta = T*(p0/P)**0.286
            potential_temperature[t,z] = theta
...
#path_to_files = 'D:/Data/'  #Home PC
path_to_files = '/Users/isaac/Desktop/Energy_Meteorology/Coding/Data/'
dlfp = 'DLFP/'
dlvad = 'DLVAD/'
#dlvad = 'houdlprofwind4newsM1.c1/'
retrieval = 'houtropoeM1.c1/'
...
stares = glob.glob(path_to_files+dlfp+'*.nc')
vads = glob.glob(path_to_files+dlvad+'*.nc')
rets = glob.glob(path_to_files+retrieval+'*.nc')

# # Build the range of dates you need to loop over
#dates=np.concatenate((np.arange(20220601,20220801,1),(np.arange(20220601,20220801,1)))
dates=((np.arange(20220601,20220801,1)))
#dates=(np.arange(20211101,20211122,1))
...
for i in range(len(dates)):
    if np.all([any(str(dates[i]) in string for string in stares), 
        ...
        dates_flags[i] = True
         ...
         dates_flags[i] = False
         if any(str(dates[i]) in string for string in stares):
             print("Stares Good!")
         if any(str(dates[i]) in string for string in vads):
             print("VADS Good!")
         if any(str(dates[i]) in string for string in rets):
             print("TROPOE Good!")
...
#     # grab proper info for day where Stare, VAD, and Retrieval file are all present
    ...
    #OPTIONS 
    plot_me = False # set to true to get plots
    ...
    author_list = 'Elizabeth Smith (NOAA-NSSL); Jacob Carlin (OU-CIMMS/NSSL); Isaac Medina (JHU)'
    contact_list = 'elizabeth.smith@noaa.gov; jacob.carlin@noaa.gov; imedina2@jhu.edu'
    campaign = 'CELO_BL'
    ...
    #files
    path_to_write = '/Users/isaac/Desktop/BLData/'
    #path_to_write = 'D:/Data/Composite/' #PC#
    ...
    date_label = date_files[-20:-12]
    platform = str("ARM") #date_files[0:4]
    #Case=date_label
    ...
    Z = (dl.variables['height'][:].data)/1000 #278 gets us up to ~5km
    Z[np.where(Z>100)] = np.nan
    lower_limit = np.ceil(Z[0]*1000.)/1000. # rounding the lowest Z value to the upper 100 m value
    lowest_gate = Z[1] #min usable range gate 
    HR = (dl.variables['time_offset'][:].data)/3600. #avg dt is 42.7917 thus 5 minutes=7.0109 records; 10 minutes=14.0214
    HR[np.where(HR>100.) ]= np.nan 
    t_s = HR * 60.0 # minutes
    snr = dl.variables['intensity'][:,:].data
    bscat = np.log10(dl.variables['backscatter'][:,:].data)
    bscat[np.where(bscat>100)] = np.nan
    w = dl.variables['radial_velocity'][:,:].data
    w[np.where(w>100)] = np.nan
    w[np.where(w==-9999)] = np.nan
    dl.close()
  #  print(HR)
...
# =============================================================================
#     plt.plot(w[0::10,0::5],snr[0::10,0::5],'bo',alpha=.3)
#     plt.xlim(-20,20)
#     plt.xlabel('w')
#     plt.ylim(1,1.02)
#     plt.ylabel('SNR')
#     plt.axhline(1.01,color='lightblue') #typical rule of thumb value
#     plt.axhline(1.009,color='lightblue') #typical rule of thumb value
...
# =============================================================================
    ...
    try:
        time_end = np.where((dlv.variables['time_offset'][:].data)/3600 >25)[0][0]
    except:
        time_end=-1
   #278 gets us up to ~5km
    #snr_cutoff=0.08#dlv['snr_threshold'][0]#+0.01
    
   # Zv[np.where(Z100)] = np.nan
    HRv = (dlv.variables['time_offset'][:time_end].data)/3600. 
    t_v = HRv * 60.0 # minutes
    
    ws = dlv.variables['wind_speed'][:time_end,:].data
    wd = dlv.variables['wind_direction'][:time_end,:].data
    
    snrv = dlv.variables['intensity'][:time_end,:].data
    snr_cutoff=1.02
    Zv = (dlv.variables['height'][:].data) 
    
# =============================================================================
#     snrv = dlv.variables['mean_snr'][:time_end,:].data
#     snr_cutoff = 0.08
#     Zv = (dlv.variables['height'][:].data) /1000
#     
# =============================================================================
...
# =============================================================================
#     plt.plot(ws[0::10,0::5],snrv[0::10,0::5],'bo',alpha=.3)
#     plt.xlim(-20,20)
#     plt.xlabel('u')
#     plt.ylim(1,1.02)
#     plt.ylabel('SNR')
#     plt.axhline(snr_cutoff,color='lightblue') #typical rule of thumb value
#     print(snr_cutoff)
#     plt.axhline(1.008,color='lightblue') #typical rule of thumb value
#     plt.title(str(platform)+' '+str(date_label))
#     plt.show()
# =============================================================================
    ...
    Za = (aeri.variables['height'][:].data)#/1000
    T_orig = (aeri.variables['temperature'][:,:].data)#-273.15
    pt_orig = Pot_Temp(aeri.variables['temperature'][:,:],aeri.variables['pressure'][:,:])
    ...
    #HFw = HFw*1000
    HFwVar[np.where(HFwVar<.1)]=np.nan #this was pulling first guess down with all zeros otherwise
    #HFwVar[np.where(HFwVar<.0001)]=np.nan 
    ...
    if plot_me == True:
        ...
        #########################################################
        plt.figure(figsize=(18,6))
        plt.pcolormesh(x,y,sum_fuzz.transpose(),vmin=0,vmax=1,cmap='inferno', shading='auto')
 ...
 #%%   
    if plot_me == True:
        ...
        #########################################################
        plt.figure(figsize=(18,6))
        plt.pcolormesh(x,y,sum_fuzz_2.transpose(),vmin=0,vmax=1,cmap='inferno', shading='auto')
...
#%%    
    ...
    if plot_me == True:
        #snr
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HR,Z,snr.transpose(),vmin=.99,vmax=1.1,cmap='inferno', shading='auto')
        ...
        #w
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HR,Z,w.transpose(),vmin=-2.5,vmax=2.5,cmap='cmo.balance', shading='auto')
        plt.colorbar(label='Intensity [SNR+1]')
        # plt.fill_between(x/60.,BLhgt_2_sm-BLhgt_2_sm_range,BLhgt_2_sm+BLhgt_2_sm_range,color='grey',alpha=.3)
        # plt.plot(x/60.,BLhgt_2_sm-BLhgt_2_sm_range,color='grey',lw=2)
        # plt.plot(x/60.,BLhgt_2_sm+BLhgt_2_sm_range,color='grey',lw=2)
        ...
        #sigsnr
        plt.figure(figsize=(18,6))
        plt.pcolormesh(sigsnr_t,Z,sigsnr.transpose(),vmin=0,vmax=.0001,cmap='inferno', shading='auto')
        ...
        plt.pcolormesh(sigw_t,Z,sigw.transpose(),vmin=0,vmax=.6,cmap='summer', shading='auto')
        ...
        #%%   
         #sigw_night
        plt.figure(figsize=(18,6))
        plt.pcolormesh(sigw_t,Z,sigw.transpose(),vmin=0,vmax=.5,cmap='summer', shading='auto')
        plt.colorbar(label='Vertical Velocity Variance [m/s]')
        plt.ylim(0.06,2.75)
# =============================================================================
...
# =============================================================================
        ...
        plt.xticks(np.arange(720,1440.1,180),x_tick_labels[4:])
        plt.xlim(720,1440)
        ...
        #sigw_HF
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HFt,Z,HFwVar.transpose(),vmin=0,vmax=.6,cmap='summer', shading='auto')
        ...
        #u
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HRv,Zv,u.transpose(),vmin=-5,vmax=15,cmap='cmo.matter', shading='auto')
        ...
        #v
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HRv,Zv,v.transpose(),vmin=-5,vmax=15,cmap='cmo.matter', shading='auto')
        ...
        #T
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HRa,Za,T.transpose(),vmin=10,vmax=30,cmap='plasma', shading='auto')
        ...
        # #WV
        plt.figure(figsize=(18,6))
        plt.pcolormesh(HRa,Za,wv.transpose(),vmin=0,vmax=18,cmap='cmo.deep', shading='auto')
...
#%%    
    ...
    if write_me==True:  
        ...
        output_file.createDimension('z', len(Z)) #time dimension, 10min
        output_file.createDimension('t_sig', len(sigw_t)) #time dimension, 10min
        ...
        output_file.createVariable('z','f8',('z'))[:] = Z
        setattr(output_file.variables['z'],'units','km')
        setattr(output_file.variables['z'],'description','Height axis in Km')
        
        output_file.createVariable('t_sig','f8',('t_sig'))[:] = sigw_t
        setattr(output_file.variables['t_sig'],'units','seconds since 00Z UTC/basetime')
        setattr(output_file.variables['t_sig'],'description','time axis for sig-w 10-minute data')
        ...
        #Sig w for stuff
        output_file.createVariable('sig_w','f4',('t_sig','z'))[:] = sigw[:,:]
        setattr(output_file.variables['sig_w'],'units','m/s')
        setattr(output_file.variables['sig_w'],'description','Veritcal Velocity Variance')
