...
Temp_Min_index = np.where(Grad_Temp==np.min(Grad_Temp))[0][0]
...
SB_t = (MWR_Data['time'][WV_Max_index]+Lidar_Data['time'][WDir_Max_index])/2
...
plt.axvline(Lidar_Data['time'][WDir_Max_index]/3600,color='grey',ls='--',lw=3)
ax.text(Lidar_Data['time'][WDir_Max_index]/3600,13.4,'t= %1.3fh'%((Lidar_Data['time'][WDir_Max_index]/3600)))
...
plt.axvline(MWR_Data['time'][WV_Max_index]/3600,color='grey',ls='--',lw=3)
ax.text(MWR_Data['time'][WV_Max_index+10]/3600,3.4,'t= %1.3fh'%((MWR_Data['time'][WV_Max_index]/3600)))
ax.set_title(Title)
if Plot_Show == True:
    plt.show()
else:
...
ax1.plot(MWR_Data['time'][Temp_Min_index]/3600,np.min(Grad_Temp),marker='o',markersize=20,color='r')
...
ax.axvline(Aero_Data['time'][sb_index]/3600,lw=4,color='grey',ls='--',zorder=0)
ax.text(Aero_Data['time'][sb_index+1000]/3600,3.8*1e4,"SB Passage at")
ax.text(Aero_Data['time'][sb_index+1000]/3600,3.6*1e4," t=%1.4fh"%(Aero_Data['time'][sb_index]/3600))


ax.axvline(Aero_Data['time'][st_aero_index]/3600,lw=4,color='lightcoral',ls='--',zorder=0)
ax.text(Aero_Data['time'][st_aero_index+100]/3600,3.8*1e4,"Sunrise at")
ax.text(Aero_Data['time'][st_aero_index+100]/3600,3.6*1e4," t=%1.4fh"%(Aero_Data['time'][st_aero_index+1000]/3600))


#ax.axvline(Lidar_Data['time'][WDir_Max_index]/3600,lw=4,color='darkseagreen',ls='--',zorder=0)
ax.set_ylim(-2000,4*1e4)
ax.text(0,-500,"Pre-Dawn Average Aerosol",fontsize=35)
ax.text (0,-1700,"Concentration: %1.2f"%(Ave_areo_Night),fontsize=35)
#ax.set_ylim()
...
ax.text(Aero_Data['time'][st_aero_index+100]/3600,-500,"Pre-SB Average Aerosol",fontsize=35)
ax.text (Aero_Data['time'][st_aero_index+100]/3600,-1700,"Concentration: %1.2f"%(Ave_aero_Morning),fontsize=35)

ax.text(Aero_Data['time'][sb_index+100]/3600,-500,"Post-SB Average Aerosol",fontsize=35)
ax.text (Aero_Data['time'][sb_index+100]/3600,-1700,"Concentration: %1.2f"%(Ave_aero_postSB),fontsize=35)
...
ax.set_xlabel('Time - UTC [hour]',fontsize=40)
ax.set_ylabel(r'Aerosol Content [$1/cm^3$]',fontsize=40)
ax1.set_ylabel(r'Aerosol Content Gradient [$(1/cm^3)~/s$]',fontsize=40)
#plt.legend(loc='upper right')
Plot_Title = "Aerosol Content with SB Passage and Sunrise Marked: "+Date
...
#%%
f, (ax1, ax2) = plt.subplots(2, 1, sharey=False,figsize=(25,40),layout="constrained")
rc('font',weight='normal',size=45) 
ax1.plot(Lidar_Data['time'][:]/3600,Lidar_Data['wind_direction'][:,z_wind],lw=5,color='k',label='raw data')
ax1.plot(Lidar_Data['time'][:]/3600,WDir_Filt,lw=7,color='g',label='Filtered data (Lowpass)')
ax1.plot(Lidar_Data['time'][:1]/3600,(Grad_WDir[:1]),lw=5,color='g',ls='--',label=r'$\frac{d}{dt}$(Filtered data)')
ax1.legend(loc='upper left')
ax1.set_ylim(0,360)
ax1.tick_params(labelsize=40)
ax1.set_xlabel('Time - UTC [hour]',fontsize=40)
ax1.set_ylabel('Wind Direction [degrees]',fontsize=40)
ax11 = ax1.twinx() 
ax11.plot(Lidar_Data['time'][:]/3600,(Grad_WDir),lw=5,color='g',ls='--',label=r'$\frac{d}{dt}$ Filtered data')
ax11.plot(Lidar_Data['time'][WDir_Max_index]/3600,np.min(Grad_WDir),lw=5,marker='o',markersize=20,color='r')
ax11.set_ylim(-1,1)
ax11.set_ylabel(r'$d~/~dt$ (Wind Direction) [degrees/second]',fontsize=40)
ax11.axvline(Lidar_Data['time'][WDir_Max_index]/3600,color='grey',ls='--',lw=3)
ax1.text(Lidar_Data['time'][WDir_Max_index]/3600,13.4,'t= %1.3fh'%((Lidar_Data['time'][WDir_Max_index]/3600)))

Title = "Wind Direction and Direction Shift Rate: " + Date
ax1.set_title(Title)


ax2.plot(MWR_Data['time'][:]/3600,MWR_Data['vap'][:],lw=3,color='k',label='raw data')
ax2.plot(MWR_Data['time'][:]/3600,WV_Filt,lw=5,color='b',label='Filtered data (Lowpass)')
ax2.plot(MWR_Data['time'][:1]/3600,(Grad_WV[:1]+np.nanmean(MWR_Data['vap'][:])),lw=3,color='b',ls='--',label=r'$\frac{d}{dt}$ Filtered data')
#ax2.set_ylim(2.5,5.0)
ax2.legend(loc='upper left')
ax2.set_xlabel('Time - UTC [hour]',fontsize=40)
ax2.set_ylabel('Water Vapor LoS Depth [cm]',fontsize=40)
ax12 = ax2.twinx() 
ax12.plot(MWR_Data['time'][:]/3600,(Grad_WV),lw=3,color='b',ls='--',label=r'$\frac{d}{dt}$ Filtered data')
ax12.plot(MWR_Data['time'][WV_Max_index]/3600,np.max(Grad_WV),marker='o',markersize=20,color='r')
ax12.set_ylabel(r'$d~/~dt$ (Water Vapor LoS Depth) [cm/second]',fontsize=40)
Title = "Water Vapor LoS Depth and Gradients Rate: " + Date
ax2.axvline(MWR_Data['time'][WV_Max_index]/3600,color='grey',ls='--',lw=3)
ax2.text(MWR_Data['time'][WV_Max_index+10]/3600,3.4,'t= %1.3fh'%((MWR_Data['time'][WV_Max_index]/3600)))
ax2.set_title(Title)
